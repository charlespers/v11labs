...
export default function WebsiteEditor({ website }: WebsiteEditorProps) {
  ...
  const [content, setContent] = useState(website?.content || '')
  const [imageUrl, setImageUrl] = useState(website?.imageUrl || '')
  const [videoUrl, setVideoUrl] = useState(website?.videoUrl || '')
  const [tags, setTags] = useState(website?.tags || '')
  const [publishedAt, setPublishedAt] = useState(
    website?.publishedAt ? new Date(website.publishedAt).toISOString().slice(0, 16) : ''
  )
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')

  const isPublished = website?.publishedAt && new Date(website.publishedAt) <= new Date()
  const websiteUrl = website && slug ? `/websites/${slug}` : null
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = website ? `/api/admin/websites/${website.id}` : '/api/admin/websites'
  ...
  const handleUnpublish = () => {
    if (confirm('Are you sure you want to unpublish this website? It will become a draft.')) {
      setPublishedAt('')
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow">
      ...
      {isPublished && (
        <div className="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-md flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">This website is published</span>
            {websiteUrl && (
              <a
                href={websiteUrl}
                target="_blank"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
          />
          <p className="mt-1 text-sm text-gray-500">URL to website image</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
          />
          <p className="mt-1 text-sm text-gray-500">URL to website video (YouTube, Vimeo, etc.)</p>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          {isPublished && (
            <span className="text-xs text-gray-500">
              Published: {website?.publishedAt ? new Date(website.publishedAt).toLocaleString() : ''}
            </span>
          )}
        </div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          {publishedAt
            ? 'Website will be published at the selected date/time. Use "Publish Now" to publish immediately.'
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mb-2">Write an article about this website</p>
        <MarkdownEditor content={content} onChange={setContent} />
      </div>

      <div className="flex justify-between items-center pt-4 border-t border-gray-200">
        <div className="text-sm text-gray-500">
          {website ? 'Editing published website' : 'Creating new website'}
        </div>
        <div className="flex gap-3">
          ...
          >
            {saving ? 'Saving...' : isPublished ? 'Update Published Website' : 'Save Website'}
          </button>
        </div>
      </div>
    </form>
