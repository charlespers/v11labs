...
export default function ResearchEditor({ research }: ResearchEditorProps) {
  const router = useRouter()
  const [title, setTitle] = useState(research?.title || '')
  const [slug, setSlug] = useState(research?.slug || '')
  const [description, setDescription] = useState(research?.description || '')
  const [content, setContent] = useState(research?.content || '')
  const [tags, setTags] = useState(research?.tags || '')
  const [publishedAt, setPublishedAt] = useState(
    research?.publishedAt ? new Date(research.publishedAt).toISOString().slice(0, 16) : ''
  )
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')

  const isPublished = research?.publishedAt && new Date(research.publishedAt) <= new Date()
  const researchUrl = research && slug ? `/research/${slug}` : null
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = research ? `/api/admin/research/${research.id}` : '/api/admin/research'
  ...
  const handleUnpublish = () => {
    if (confirm('Are you sure you want to unpublish this research article? It will become a draft.')) {
      setPublishedAt('')
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow">
      ...
      {isPublished && (
        <div className="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-md flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">This research article is published</span>
            {researchUrl && (
              <a
                href={researchUrl}
                target="_blank"
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          {isPublished && (
            <span className="text-xs text-gray-500">
              Published: {research?.publishedAt ? new Date(research.publishedAt).toLocaleString() : ''}
            </span>
          )}
        </div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          {publishedAt
            ? 'Research article will be published at the selected date/time. Use "Publish Now" to publish immediately.'
      ...
      <div className="flex justify-between items-center pt-4 border-t border-gray-200">
        <div className="text-sm text-gray-500">
          {research ? 'Editing published research article' : 'Creating new research article'}
        </div>
        <div className="flex gap-3">
          ...
          >
            {saving ? 'Saving...' : isPublished ? 'Update Published Research' : 'Save Research'}
          </button>
        </div>
      </div>
    </form>
